...
class Log(OS):
    ...
    def __init__(self, log_path: Text, log_file: Text, log_level: Text, logger_name: Text) -> NoReturn:
        self._log_path = log_path if log_path else "/var/log/cleanup"
    ...
    def _check_log_level(level: Text) -> Text:
        try:
            ...
            print(f"\nError general exception in check log level {level} - {error}")
    ...
    def _check_log_path_and_log_file(self) -> NoReturn:
        try:
            ...
            print(f"\nError general exception when we check if log path/file {self.log_path}/{self.log_file} exist in the system - {error}")
